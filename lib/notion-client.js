...
async function createPageWithSchemaRetry(parent, properties, label, maxRetries = 5) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Only strip-and-retry if we still have retry budget left. On the
            // final iteration we let the error propagate so the caller sees
            // the real validation_error rather than a misleading "retrying…"
            // log followed by a terminal throw.
            if (missing && props[missing] !== undefined && attempt < maxRetries) {
    ...
    // Unreachable: the loop always exits via return or throw above.
    ...
    try {
        return await createPageWithSchemaRetry(
            { database_id: notionConfig.databases.fitReports },
            {
                ...
                overall_fit_score: formatNotionProperty('number', reportData.overallFitScore),
                skills_match_percent: formatNotionProperty('number', reportData.skillsMatchPercent),
                raw_skills_match_percent: formatNotionProperty('number', reportData.rawSkillsMatchPercent),
                evidence_match_percent: formatNotionProperty('number', reportData.evidenceMatchPercent),
...
export async function getFitReports(userEmail) {
    try {
        ...
        const reports = response.results.map(page => ({
            ...
            overallFitScore: getNotionProperty(page, 'overall_fit_score'),
            skillsMatchPercent: getNotionProperty(page, 'skills_match_percent'),
            rawSkillsMatchPercent: getNotionProperty(page, 'raw_skills_match_percent'),
            evidenceMatchPercent: getNotionProperty(page, 'evidence_match_percent'),
